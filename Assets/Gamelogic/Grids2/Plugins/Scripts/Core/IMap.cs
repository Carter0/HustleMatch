...
namespace Gamelogic.Grids2
{
	...
	public static class Map
	{
		...
		/// <summary>
		/// Creates a Reverse Map that swap the calls of Forward and Reverse, 
		/// meaning that now calling Forward
		/// will do the Reverse call and vice versa.
		/// </summary>
		/// <typeparam name="TInput">The type of the Input.</typeparam>
		/// <typeparam name="TOutput">The type of the Output.</typeparam>
		/// <param name="map">Base map where you apply this call.</param>
		public static IMap<TOutput, TInput> Reverse<TInput, TOutput>(
			this IMap<TInput, TOutput> map)
		{
			return new InverseMap<TOutput, TInput>(map);
		}

		/// <summary>
		/// Creates a Reverse Map based only on the InverseForwardMap.
		/// In other words only the Reverse is swapped,
		...
		/// <summary>
		/// Creates a Reverse Map based only on the InverseReverseMap. In other 
		/// words only the Forward is swapped,
		/// meaning that calling Forward will do the Reverse operation.
		/// </summary>
		/// <typeparam name="TInput">The type of the Input.</typeparam>
		/// <typeparam name="TOutput">The type of the Output.</typeparam>
		/// <param name="map">Base map where you apply this call.</param>
		public static IForwardMap<TOutput, TInput> Reverse<TInput, TOutput>(
			this IReverseMap<TInput, TOutput> map)
		{
		...
		/// <summary>
		/// Returns a Map that translate in such a way that it is aligned horizontally
		...
		[Obsolete]
		public static IMap<Vector3, Vector3> AlignGridHorizontal(
			this IMap<Vector3, Vector3> map, 
			IExplicitShape<GridPoint2> shape, 
			Func<GridPoint2, Vector3> cellSize, 
			Bounds bounds, HorizontalAlignment alignment)
		{
			var gridBounds = map.CalculateBounds(shape, cellSize, p=>p.ToVector2().To3DXY());
		...
		/// <param name="bounds">Bounds of the Map.</param>
		/// <param name="alignment">Type of alignment to apply.</param>
		public static IMap<Vector3, Vector3> AlignGridVertical(
			this IMap<Vector3, Vector3> map, 
			IExplicitShape<GridPoint2> shape, 
			Func<GridPoint2, Vector3> cellSize, 
			Bounds bounds, VerticalAlignment alignment)
		{
			var gridBounds = map.CalculateBounds(shape, cellSize, p => p.ToVector2().To3DXY());
		...
		/// <param name="shape">Shape used in the map.</param>
		/// <param name="cellSize">This function is used to calculate the size of a cell.</param>
		/// <param name="toVector">A function that can convert the point type to a Vector3</param>
		/// <param name="bounds">Bounds of the Map.</param>
		...
		public static IMap<Vector3, Vector3> AlignGridInRect<TPoint>(
			this IMap<Vector3, Vector3> map,
			IExplicitShape<TPoint> shape,
			Func<TPoint, Vector3> cellSize,
			Func<TPoint, Vector3> toVector,
			Bounds bounds,
			HorizontalAlignment horizontalAlignment,
			VerticalAlignment verticalAlignment)
		{
			var gridBounds = map.CalculateBounds(shape, cellSize, toVector);
		...
		//TODO: should this not really take a GridMap instead?
		/// <summary>
		/// Calculates the world bounds of an explicit shape.
		/// </summary>
		/// <typeparam name="TPoint">The point type of the shape.</typeparam>
		/// <param name="gridToWorld">The map that represents the transform from 
		/// grid point to world point.</param>
		/// <param name="shape">The shape whose bounds to calculate.</param>
		/// <param name="cellSize">A function that gives the dimensions of a cell at a given point
		/// in world space.</param>
		/// <param name="toVector">A function that can turn a grid point into a Vector3.</param>
		/// <remarks>
		/// <para>
		/// The bounds are guaranteed to be tight.
		/// </para>
		/// </remarks>
		public static Bounds CalculateBounds<TPoint>(
			this IMap<Vector3, Vector3> gridToWorld, 
			IExplicitShape<TPoint> shape, 
			Func<TPoint, Vector3> cellSize,
			Func<TPoint, Vector3> toVector)
		{
			...
			var worldPoint = gridToWorld.Forward(toVector(firstPoint));
			...
			foreach (var point in shape.Points.ButFirst())
			{
				...
				worldPoint = gridToWorld.Forward(toVector(point));
		...
		/// <param name="cellSize">This function is used to calculate the size of a cell.</param>
		/// <param name="alignment">Type of alignment to apply.</param>
		public static IMap<Vector3, GridPoint3> AnchorPivotHorizontal(
			this IMap<Vector3, GridPoint3> map, 
			IExplicitShape<GridPoint3> shape, 
			Func<GridPoint3, Vector3> cellSize, 
		...
		/// <param name="cellSize">This function is used to calculate the size of a cell.</param>
		/// <param name="alignment">Type of alignment to apply.</param>
		public static IMap<Vector3, GridPoint3> AnchorPivotVertical(
			this IMap<Vector3, GridPoint3> map, 
			IExplicitShape<GridPoint3> shape, 
			Func<GridPoint3, Vector3> cellSize, 
		...
		/// <preliminary/> 
		public static IMap<Vector3, Vector3> StretchInRectWidthProportional(
			this IMap<Vector3, Vector3> map, 
			IExplicitShape<GridPoint2> shape, 
			Func<GridPoint2, Vector3> cellSize, 
			Bounds rect)
		...
		/// <preliminary/> 
		public static IMap<Vector3, Vector3> StretchInRectHeightProportional(
			this IMap<Vector3, Vector3> map, 
			IExplicitShape<GridPoint2> shape, 
			Func<GridPoint2, Vector3> cellSize, Bounds bounds)
		{
			var gridBounds = map.CalculateBounds(shape, cellSize, p => p.ToVector2().To3DXY());
			//TODO check
		...
		/// <param name="bounds">Bounds of the screen where the grid is.</param>
		/// <preliminary/> 
		public static IMap<Vector3, Vector3> StretchInRect(
			this IMap<Vector3, Vector3> map, 
			IExplicitShape<GridPoint2> shape, 
			Func<GridPoint2, Vector3> cellSize, 
			Bounds bounds)
		...
		/// <param name="bounds">Bounds of the screen where the grid is.</param>
		/// <preliminary/> 
		public static IMap<Vector3, Vector3> StretchInRectToFit(
			this IMap<Vector3, Vector3> map, 
			IExplicitShape<GridPoint2> shape, 
			Func<GridPoint2, Vector3> cellSize, 
			Bounds bounds)
		...
		/// </summary>
		/// <param name="func">The function that will be applied to the X coordinate.</param>
		...
		/// <summary>
		/// Returns a map that wraps the X coordinate around at the given width and the
		/// Y coordinate around the given height.
		/// </summary>
		...
		//TODO: @herman
		public static IMap<Vector3, GridPoint2> PolygonOffset(
			this IMap<Vector3, GridPoint2> roundMap, 
			IPointPartition2 partition, 
			IList<GridPoint2> offsets)
		{
		...
		/// </summary>
		/// <param name="anglePerRadius">How much a point should be rotated is this value times
		/// the distance of a point from the origin.</param>
		...
		private static Vector3 Twirl(Vector3 input, float anglePerRadius)
		{
		...
		private static Vector3 TwirlReverse(Vector3 output, float anglePerRadius)
		{
