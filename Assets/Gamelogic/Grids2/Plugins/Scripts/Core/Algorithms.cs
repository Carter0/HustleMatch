...
namespace Gamelogic.Grids2
{
	...
	public static class Algorithms
	{
		...
		//TODO: Implement general graph iteration algorithms
		/// <summary>
		...
		/// <param name="neighborToNeighborCost">This function is used to calculate the cost of moving to a given neighbor.</param>
		/// <returns>The list of nodes on the path in order.If no path is possible, null is returned.</returns>
		/// <remarks>
		/// For detailed documentation on how this method works, see <conceptualLink target="fa258ee3-b3ad-4129-9d06-9d51b58ea255"/>
		/// </remarks>
		...
		/// <param name="neighborToNeighborCost">This function is used to calculate the cost of moving to a given neighbor.</param>
		/// <returns>The list of nodes on the path in order. If no path is possible, null is returned.</returns>
		//TODO: Didn't touch this when optimizing the algorithms but should be the same as 
		//AStar above (@omarrojo)
		...
		/// <remarks>
		/// <note type="note">Author: Justin Kivak</note>
		/// <para>See <conceptualLink target="3e22bb01-63e3-410f-8dcb-08d8454fb638"/> for detailed
		/// information on how to use this method.</para>
		/// </remarks>
		...
		/// <param name="isAccessible">This function is used to know if a point is accessible within the grid.</param>
		...
		/// <summary>
		/// Returns a list containing lines connected to the given points. A line is a list of
		/// points. Only returns correct results for square or hex grids.
		/// </summary>
		/// <example>
		/// <code>
		/// <![CDATA[
		...
		/// <param name="rayGenerators">A list if forward maps used to generate rays.</param>
		/// <param name="isNeighborsConnected">
		/// A functions that returns true or false, depending on whether two points can be considered
		/// connected when they are neighbors.For example, if you want rays of points that refer to
		/// cells of the same color, you can pass in a functions that compares the DefaultColors of cells.
		/// </param>
		...
		/// <summary>
		/// Creates a new grid where the neighbors of a point are 
		/// aggregated to the grid changing the type of the points.
		/// </summary>
		...
		/// <example>
		/// <para> 
		/// Suppose we have a grid with a count of enemies in each cell and
		/// we want a grid that has in each cell, the sum of the enemies
		/// in neighboring cells. We can use the following to do that:
		/// </para>
		/// <code>
		/// <![CDATA[
		...
		/// <inheritdoc cref="AggregateNeighborhood{TPoint, TResultCell}(IGrid{TPoint}, Func{TPoint, IEnumerable{TPoint}}, Func{IEnumerable{TPoint}, TResultCell})"/>
		public static void
			AggregateNeighborhood<TPoint, TCell>(
				IGrid<TPoint, TCell> grid,
				Func<TPoint, IEnumerable<TPoint>> getNeighborHood,
		...
		//TODO Define for 1D and 2D and make public
		[Version(2, 3, 9)]
		private static IGrid<GridPoint2, TResultCell>
			AggregateNeighborhood<TResultCell>(
				IGrid<GridPoint2> grid,
				IExplicitShape<GridPoint2> neighborHood,
				Func<IEnumerable<GridPoint2>, TResultCell> aggregator)
		{
			return AggregateNeighborhood(
				grid,
				p => neighborHood.Points.Select(q => p + q),
				aggregator);
		}
		#endregion
